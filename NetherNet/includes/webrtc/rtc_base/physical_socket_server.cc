...
int PhysicalSocket::SetOption(Option opt, void const* value, size_t size){
  int slevel;
  int sopt;
  if(TranslateOption(opt, &slevel, &sopt) == 1)
    return -1;
  int result = ::setsockopt(s_, slevel, sopt, (char*)value, size);
  if(result != 0){
    UpdateLastError();
  }
  return result;
...
int PhysicalSocket::TranslateOption(Option opt, int* slevel, int* sopt) {
  switch (opt) {
    ...
    case OPT_DSCP:
      ...
      return -1;
#endif
    case OPT_SO_BROADCAST:
      *slevel = SOL_SOCKET;
      *sopt = SO_BROADCAST;
      break;
    case OPT_IPV6_JOIN_GROUP:
      *slevel = IPPROTO_IPV6;
      *sopt = IPV6_JOIN_GROUP;
      break;
    case OPT_IPV6_MULTICAST_LOOP:
      *slevel = IPPROTO_IPV6;
      *sopt = IPV6_MULTICAST_LOOP;
      break;
    case OPT_SEND_ECN:
#if defined(WEBRTC_POSIX)
      if (family_ == AF_INET6) {
