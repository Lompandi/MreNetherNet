...
namespace NetherNet {
	...
	bool PeerRecordTable::AddOrUpdate(NetworkID networkID, rtc::SocketAddress const& address) {
		...
		if (record == mPeerRecordTable.end()) {
			...
			NetherNet::NetherNetTransport_LogMessage(4, "[LAN] Adding [%llu] at '%s' to database", networkID, str_addr.data());
			...
			if(address == record->second.mRemoteAddress)
				NetherNet::NetherNetTransport_LogMessage(4, "[LAN] Updating database for [%llu]", networkID);
			else {
